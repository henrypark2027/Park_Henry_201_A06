...
[https://github.com/henrypark2027/Park_Henry_201_A06/tree/main](https://github.com/henrypark2027/Park_Henry_201_A06/tree/main)
...
```{r}
player_info$MJ <- append(player_info$MJ, list(height=78))
player_info$LJ <- append(player_info$LJ, list(height = 81))
player_info$KB <- append(player_info$KB, list(height = 78))
player_info$KA <- append(player_info$KA, list(height = 86))
player_info$SO <- append(player_info$SO, list(height = 85))
...
```{r}
# get points per game for each player
points_per_game <- list()
for (athlete in player_info) {
  points_per_game <- append(points_per_game, athlete$points/athlete$games_played)
}
# collect player names in a vector
names <- c()
for (athlete in player_info) {
  names <- append(names, athlete$name)
}
# assign names vector to points_per_game
names(points_per_game) <- names
...
```{r}
# round function 
round_num <- function(my_num) {
  return(round(my_num, digits=2))
}

# we have a list, so we should use lapply 
lapply(points_per_game, round_num)
```
## Question 3 (3 points)
...
round_fun <- function(x) {
  ...
  round(x, digits=-(num_digits-1)) 
  # ex) num_digits = nchar("262") = 3. 
  # But, we want to round to the nearest hundred (which is digits = -2), so
  # we want to subtract 1 from num_digits. 
}

# We have a matrix, so we want to use APPLY 
print(apply(mat_rand, 1:2, round_fun))
```
## Question 4 (9 points)
...
```{r}
sum_digits <- function(my_num) {
  if (my_num == 0) { # base case
    return(0)
  } else { # recursive case
    return(my_num%%10+sum_digits(my_num%/%10)) 
    # ^^ get remainder and truncate my_num by 10
  }
}

print(sum_digits(935068))
```
